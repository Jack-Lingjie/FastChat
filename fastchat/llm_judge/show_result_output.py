...
def display_result_single(args):
    ...
    os.makedirs('data/result/', exist_ok=True)  # 创建目录  
    df_1.sort_values(by="score", ascending=False).to_csv('data/result/first_turn.csv', index=True)  # 保存为CSV文件  

    if args.bench_name == "mt_bench":
        print("\n########## Second turn ##########")
        df_2 = df[df["turn"] == 2].groupby(["model", "turn"]).mean()
        print(df_2.sort_values(by="score", ascending=False))
        df_2.sort_values(by="score", ascending=False).to_csv('data/result/second_turn.csv', index=True)  # 保存为CSV文件  

        print("\n########## Average ##########")
        df_3 = df[["model", "score"]].groupby(["model"]).mean()
        print(df_3.sort_values(by="score", ascending=False))
        df_3.sort_values(by="score", ascending=False).to_csv('data/result/average.csv', index=True)  # 保存为CSV文件  
